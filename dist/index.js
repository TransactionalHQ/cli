...
// src/lib/types.ts
var CliSessionType = /* @__PURE__ */ ((CliSessionType2) => {
  CliSessionType2["CLI"] = "CLI";
  CliSessionType2["MCP"] = "MCP";
  return CliSessionType2;
})(CliSessionType || {});
...
async function emailInput(message, options) {
...
async function editor(message, options) {
  const { value } = await inquirer.prompt([
    {
      type: "editor",
      name: "value",
...
function createLoginCommand() {
  return new Command("login").description("Authenticate with Transactional").option("--mcp", "Login for MCP server use").option("-f, --force", "Force new login even if already logged in").action(async (options) => {
    if (isLoggedIn() && !options.force) {
      ...
      if (currentOrg) {
        printInfo2(`Already logged in. Current organization: ${currentOrg}`);
      } else {
        printInfo2('Already logged in. Use "transactional org use <slug>" to select an organization.');
      }
      printInfo2('Use "transactional login --force" to get a new token.');
      return;
    }
    const spinner = ora2();
    ...
    if (result.data.organizations.length > 0) {
      console.log();
      printInfo2(`You have access to ${result.data.organizations.length} organization(s).`);
      printInfo2('Use "transactional org list" to see them, or "transactional org use <slug>" to select one.');
    }
  });
}
...
function createLogoutCommand() {
  return new Command("logout").description("Log out from all organizations").action(async () => {
    if (!isLoggedIn()) {
      printInfo2("You are not logged in.");
      return;
    }
    logout();
  ...
  return new Command("whoami").description("Show current user and organization info").option("-o, --org <slug>", "Organization slug").option("--json", "Output as JSON").action(async (options) => {
    ...
    if (options.json) {
      ...
      if (organization) {
        ...
        printInfo2('No organization selected. Use "transactional org use <slug>" to select one.');
      }
      console.log();
      printHeading2("Session");
  ...
  return new Command("switch").description('Switch to a different organization (alias for "org use")').argument("[slug]", "Organization slug to switch to").action(async (slug) => {
    ...
    if (!targetSlug) {
      if (orgs.length === 1) {
        ...
        printInfo2(`Only one organization available: ${targetSlug}`);
      } else {
    ...
    if (!validOrg) {
      ...
      printInfo2("Available organizations: " + orgs.map((o) => o.slug).join(", "));
      process.exit(1);
    }
...
function createOrgsCommand() {
  ...
  orgCmd.command("list").description("List all organizations you have access to").option("--json", "Output as JSON").action(async (options) => {
    ...
    const currentOrg = getCurrentOrganization();
    if (orgs.length === 0) {
      printInfo2("No organizations found.");
      return;
    }
    ...
    if (options.json) {
      ...
      print(orgInfos);
      console.log();
      printInfo2(`Current organization: ${currentOrg || "(none selected)"}`);
    }
  });
  orgCmd.command("use").description("Set the current organization for CLI commands").argument("<slug>", "Organization slug to use").action(async (slug) => {
    ...
    if (!org) {
      ...
      printInfo2('Use "transactional org list" to see available organizations.');
      process.exit(1);
    }
    spinner.stop();
...
function createConfigCommand() {
  ...
  configCmd.command("show").description("Show current configuration").option("--json", "Output as JSON").action((options) => {
    ...
    if (options.json) {
      ...
      console.log();
      printHeading3("Current Configuration");
...
function printWarning2(message) {
  if (isColorEnabled()) {
      ...
      try {
        if (t === "claude-desktop") {
          ...
          if (existingConfig.mcpServers?.transactional && !options.force) {
            printWarning2(`Claude Desktop: Already configured. Use --force to overwrite.`);
...
`);
          ...
          if (existingConfig.mcpServers?.transactional && !options.force) {
            printWarning2(`Claude Code: Already configured. Use --force to overwrite.`);
            console.log(`  Config: ${configPath}
`);
            anySkipped = true;
            continue;
...
`);
  ...
  mcpCmd.command("uninstall").description("Remove MCP config from Claude Desktop and/or Claude Code").option("--target <target>", "Target: claude-desktop, claude-code, both", "both").action((options) => {
    ...
    console.log("");
    printWarning2("Please restart Claude Desktop/Code to apply changes.\n");
  });
